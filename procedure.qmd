...
![This figure depicts the design of an example trial. At familiarization, a single image is shown for 5, 10, or 15s. After the specified familiarization time is reached (based on the *infant-controlled* or *fixed-length* design criteria), infants are presented with a central fixation stimulus for 500ms. During the two test phases (5s each), infants view the same image to which they were familiarized paired with a new stimulus from the same set (e.g., *Fribbuli* or *Fractals*) and the same level of complexity (*low* or *high*). The side on which the familiar and novel images are presented is counterbalanced across test phases within a trial, but the images remain the same.](images/mb5-trial-overview.png){#fig-trial}
